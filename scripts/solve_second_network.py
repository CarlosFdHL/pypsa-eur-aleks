...
def fix_networks(
    n: pypsa.Network,
    fixed_network: pypsa.Network,
) -> None:
    """
    Fix networks (DC, AC, CO2, gas, H2) of `n` to optimized capacities of `fixed network`.

    Use this function when a capacity expansion optimization has already been performed and other capacities are to be optimized.

    Parameters
    ----------
    n : pypsa.Network
        PyPSA network whose links (DC, AC, CO2, gas, H2) are to be fixed
    fixed_network : pypsa.Network
        PyPSA network with optimized capacities to fix `n` to
    """
    # TODO: check if this works for PyPSA V1
    if PYPSA_V1:
        logging.warning("fix_networks is experimental for PyPSA v1")
        for comp, attr in {
            "Line": "s_nom",
            "Link": "p_nom",
        }.items():
            ...
            if comp == "Line":
                ext_i = ext_i.intersection(
                    n.lines[n.lines.carrier == "AC"].index
                )
            elif comp == "Link":
                ext_i = ext_i.intersection(n.links[n.links.carrier.isin(["DC", "H2 pipeline", "gas pipeline", "gas pipeline new", "CO2 pipeline"])].index)
            c.static[attr].loc[ext_i] = fixed_network.components[comp].static.loc[ext_i, attr + "_opt"]
            c.static.loc[ext_i, attr + "_extendable"] = False
    else:
        logging.info("Fixing network capacities to optimized capacities from fixed network.")
        for comp, attr in {
            "Line": "s_nom",
            "Link": "p_nom",
        }.items():
            ext_i = n.get_extendable_i(comp)
            if comp == "Line":
                ext_i = ext_i.intersection(
                    n.lines[n.lines.carrier == "AC"].index
                )
            elif comp == "Link":
                ext_i = n.get_extendable_i("Link").intersection(n.links[n.links.carrier.isin(["DC", "H2 pipeline", "gas pipeline", "gas pipeline new", "CO2 pipeline"])].index)
            n.static(comp).loc[ext_i, attr] = fixed_network.static(comp).loc[ext_i, attr + "_opt"]
            n.static(comp).loc[ext_i, attr + "_extendable"] = False
...
if __name__ == "__main__":
    ...
    fixed_network = pypsa.Network(snakemake.input.fixed_network)
